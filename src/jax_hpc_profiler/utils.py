...
def plot_with_pdims_strategy(
    ...
    if "plot_fastest" in pdims_strategy:
        ...
        label_params.update(
            {"decomposition": f"{group['px'].values[0]}x{group['py'].values[0]}"}
        )
        ...
        for _, group in df_decomp:
            ...
            if (
                "plot_all" not in pdims_strategy
                and group["decomp"].values[0] not in pdims_strategy
            ):
                continue
...
def clean_up_csv(
    ...
    available_data_sizes = (
        ...
        if data_sizes is None
            ...
            data_size for data_size in data_sizes if data_size in available_data_sizes
        ]
    )
